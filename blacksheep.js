...
class Blacksheep {
  ...
  onScreen() {
    return this.x - BLACKSHEEP_SIZE < cwidth / 2 && -cwidth / 2 < this.x + BLACKSHEEP_SIZE
      && this.y - BLACKSHEEP_SIZE < cheight / 2 && -cheight / 2 < this.y + BLACKSHEEP_SIZE * 1.5;
  }

  drawSheep(context) {
    if (this.onScreen()) {
      context.moveTo(this.x + BLACKSHEEP_SIZE, this.y + this.z);
  ...
  drawShadow(context) {
    if (this.onScreen()) {
      context.moveTo(this.x + BLACKSHEEP_SIZE, this.y + BLACKSHEEP_SIZE);
      context.ellipse(this.x, this.y + BLACKSHEEP_SIZE, BLACKSHEEP_SIZE, BLACKSHEEP_SIZE / 2, 0, 0, FULL_CIRCLE);
    }
  }
  ...
  move() {
    if (this.noCheck) this.noCheck = false;
      ...
      if (dx === 0 && dy === 0) {
        ...
        this.yv += Math.sin(angle) * BLACKSHEEP_SIZE;
        return true;
      } else if (dx * dx + dy * dy < 4 * BLACKSHEEP_SIZE * BLACKSHEEP_SIZE) {
        [shep.xv, this.xv] = [this.xv, shep.xv];
        [shep.yv, this.yv] = [this.yv, shep.yv];
        shep.noCheck = true;
        const dist = Math.sqrt(dx * dx + dy * dy);
        const angle = Math.atan2(dy, dx);
        this.x -= Math.cos(angle) * (BLACKSHEEP_SIZE * 2 - dist + 1e-5);
        this.y -= Math.sin(angle) * (BLACKSHEEP_SIZE * 2 - dist + 1e-5);
        return true;
      } else if (this.blacksheep && dx * dx + dy * dy < 64 * BLACKSHEEP_SIZE * BLACKSHEEP_SIZE) {
...
function drawBlacksheep() {
  ...
  c.fillStyle = 'black';
  c.beginPath();
  sheep.forEach(s => s.drawBlacksheep(c));
  c.fill();
}
function moveBlacksheep() {
